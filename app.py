...
with st.sidebar:
    ...
    st.markdown("**OR**")
    website_url = st.text_input("🌐 Enter a website URL", placeholder="https://example.com/article")
...
# =========================================================
# BUILD RAG CHAIN (uses a website URL via WebBaseLoader)
# =========================================================
@st.cache_resource(show_spinner=False)
...
if uploaded_file is not None:
    ...
    if st.session_state.get("uploaded_file_hash") != file_hash:
        ...
        # Hash-based ID: same file content => same store => no re-embedding
        # on repeat uploads, even across different sessions.
        db_session_id = file_hash

        status = st.status("📚 Processing PDF...", expanded=True)
        status.write("📄 Loading and parsing PDF pages...")
        status.write("✂️ Splitting document into chunks...")
        status.write("🧠 Generating embeddings (this is the slow step — one API call per chunk)...")
        status.write("🗂️ Building vector store...")
...
# =========================================================
if website_url:
    if st.session_state.get("uploaded_filename") != website_url:
        with st.spinner("🌐 Processing website — loading, chunking & embedding content..."):
            if "session_id" not in st.session_state:
                import uuid
                st.session_state.session_id = str(uuid.uuid4())
            db_session_id = f"{st.session_state.session_id}_{website_url}"

            retriever, template, LLM, parser, get_context = build_chain_from_url(website_url, db_session_id)
            ...
            st.session_state.get_context = get_context
            st.session_state.chain = True
            st.session_state.uploaded_filename = website_url
            st.session_state.messages = []

        st.success(f"✅ Website content processed successfully! You can start asking questions.")
